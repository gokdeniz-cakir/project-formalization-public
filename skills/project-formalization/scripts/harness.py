#!/usr/bin/env python3
"""Deterministic helpers for blind C formal-verification experiment harnesses."""
...
FORMAL_FAMILIES = {
    ...
    "configurable_program_analysis",
    "horn_clause_verification",
    "symbolic_execution_with_completeness",
}
NON_FORMAL_TOOLS = {
    "asan",
    "addresssanitizer",
...
def validate_formal_configuration(data: dict[str, Any], errors: list[str], label: str) -> None:
    if nested(data, "workflow.method_policy") != "formal_required":
        errors.append(f"{label} requires workflow.method_policy=formal_required")
    if nested(data, "target.language") != "c":
        errors.append(f"{label} requires target.language=c")
    if nested(data, "tooling.formal.required") is not True:
        errors.append(f"{label} requires tooling.formal.required=true")
    name = nested(data, "tooling.formal.name")
    if not isinstance(name, str) or not name.strip():
        errors.append("tooling.formal.name must identify a formal backend")
    elif name.strip().lower() in {"investigator_choice", "select-after-preflight"}:
        errors.append("tooling.formal.name must be selected before formal baseline validation")
    elif name.strip().lower() in NON_FORMAL_TOOLS:
        errors.append("tooling.formal.name must be a machine-checking backend, not a runtime oracle")
    family = nested(data, "tooling.formal.family")
    if family not in FORMAL_FAMILIES:
        errors.append(f"tooling.formal.family must be one of: {', '.join(sorted(FORMAL_FAMILIES))}")
    require_nonempty(errors, data, "tooling.formal.execution_environment")
    require_nonempty(errors, data, "tooling.formal.version_command")
    require_nonempty(errors, data, "tooling.formal.analyze_command")
    require_nonempty(errors, data, "tooling.formal.property_mapping")
    properties = data.get("properties")
    if not isinstance(properties, list) or not properties:
        errors.append("formal verification requires at least one safety property")
...
def validate_freeze(data: dict[str, Any]) -> list[str]:
    ...
    validate_formal_configuration(data, errors, "formal baseline")
    for field in (
        "experiment.id",
        "experiment.mode",
        "project.name",
    ...
    placeholder_values = {
        ...
        "target.component": ("bounded component", "bounded c component"),
...
def validate_baseline(repo: Path) -> list[str]:
    ...
    validate_formal_configuration(data, errors, "formal investigator baseline")
    for field in (
        "experiment.id",
        "experiment.mode",
        "project.name",
...
def require_formal_receipt(errors: list[str], data: dict[str, Any], require_bounds: bool = False) -> None:
    for field in (
        "formal.tool",
        "formal.family",
        "formal.version",
        "formal.execution_environment",
        "formal.commands",
        "formal.exit_codes",
        "formal.property_ids",
        ...
        require_nonempty(errors, data, field)
    tool = str(nested(data, "formal.tool", "")).strip().lower()
    if tool in NON_FORMAL_TOOLS:
        errors.append("formal.tool must identify a machine-checking backend")
    if nested(data, "formal.family") not in FORMAL_FAMILIES:
        errors.append("formal.family must identify a supported formal method family")
    if require_bounds:
        require_nonempty(errors, data, "formal.bounds")
...
def validate_result(data: dict[str, Any], root: Path) -> list[str]:
    ...
    if verdict not in VERDICTS:
        errors.append("verdict must be confirmed_violation, formal_only_candidate, bounded_satisfaction, proved_under_contracts, or unknown")
    ...
    for field in (
        ...
        require_nonempty(errors, data, field)
    for field in ("formal.commands", "formal.exit_codes", "formal.property_ids", "formal.property_files", "formal.logs", "formal.production_units", "formal.models", "formal.stubs", "formal.assumptions", "formal.completeness_checks", "formal.proof_obligations", "formal.secondary_checks", "residuals", "independence.incidents"):
    ...
    if formal_status not in {"violated", "satisfied", "proved", "unknown", "tool_failure"}:
        errors.append("formal.status must be violated, satisfied, proved, unknown, or tool_failure")
    ...
    if verdict == "confirmed_violation":
        require_formal_receipt(errors, data, require_bounds=True)
        for field in (
            "snapshot_digest",
            "formal.tool",
            "formal.version",
        ...
        if not nested(data, "formal.completeness_checks"):
            errors.append("bounded_satisfaction requires explicit completeness_checks")
        if not limitations:
    ...
    elif verdict == "proved_under_contracts":
        require_formal_receipt(errors, data)
        if formal_status != "proved":
            errors.append("proved_under_contracts requires formal.status=proved")
        if not nested(data, "formal.proof_obligations"):
            errors.append("proved_under_contracts requires proof_obligations")
        if not nested(data, "formal.completeness_checks"):
            errors.append("proved_under_contracts requires completeness_checks")
        if nested(data, "formal.completeness_declared") is not True:
            errors.append("proved_under_contracts requires completeness_declared=true")
        if data.get("cause_assessment") != "not_applicable":
            errors.append("proved_under_contracts requires cause_assessment=not_applicable")
        claim_scope = str(data.get("claim_scope", "")).lower()
        if "contract" not in claim_scope and "assumption" not in claim_scope:
            errors.append("proved_under_contracts claim_scope must mention contracts or assumptions")
        validate_artifacts(data, root, errors, require_hash=True)
        validate_artifact_references(data, errors)
...
def plan_setup(freeze_path: Path, shell: str) -> list[dict[str, str]]:
    ...
    steps.append({
        "id": "commit-baseline",
        "command": command(["git", "-C", destination, "commit", "-m", "Create neutral formal-verification baseline"], shell),
    ...
    if policy == "private_baseline_once":
        ...
        steps.append({"id": "disable-push-url", "command": command(["git", "-C", destination, "remote", "set-url", "--push", "origin", "disabled://project-formalization"], shell), "checkpoint": "Origin push URL begins with disabled://."})
